...
def parse(buf):
    try:
        ...
        #import pdb; pdb.set_trace()
        print 'Error:%s:%s.%s:illegal character:%s' % (options.filename,
                                                       lineno,column,c)
        return []
    except NotImplementedError as e:
        print 'Error:%s:not implemented:%s' % (options.filename,e)
        return []
    except syntax_error as e:
        try:
            #import pdb;pdb.set_trace()
            column=e[0].lexpos - new_lexer.lexdata.rfind("\n",0,e[0].lexpos)
            print '%s:%s.%s:syntax error %s=<%s>' % (options.filename,
                                                     e[0].lineno,
                                                     column,
                                                     e[0].type,
                                                     e[0].value)
        except Exception as ex:
            print "Unexpected EOF or other error",ex
        return []
